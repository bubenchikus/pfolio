...
export const pictureValidation = [
  ...
  body("category")
    .isIn(config.get("categories.pictures"))
  ...
  body("created")
    ...
    .matches(/^SW\d{4}-\d{1,2}$/)
    .withMessage("Picture's created must match xxxx-xx pattern!"),
  body("redraw").isBoolean().withMessage("Picture's redraw must be boolean!"),
  body("hide").isBoolean().withMessage("Picture's redraw must be boolean!"),
];
