...
export async function uploadDescription(txt, category, series, arrangement) {
  const res = await userQuery(
    `
      ...
      VALUE (?,COALESCE(?, DEFAULT(category)),COALESCE(?, DEFAULT(series)),COALESCE(?,DEFAULT(arrangement)));
  `,
...
export async function updatePicturesSeries(oldSeries, newSeries, category) {
  const res = await userQuery(
    `
      UPDATE picture
