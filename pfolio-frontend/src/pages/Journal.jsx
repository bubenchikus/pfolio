...
export const Journal = () => {
  ...
  <div className={universalStyles.buttonBox}>
    {Object.keys(categories).map((category) => <div key={category} id={category} 
    className={category === "all" ? universalStyles.buttonPressed : universalStyles.button} 
    onClick={() => {
  ...
  </Container>
   {
    ...
    ? (
    categories["all"] ?
    categories["all"].filter((el) => el.journalIndex >= (currentPage - 1) * postsPerPage && el.journalIndex < currentPage * postsPerPage)
    ?.map((post) => 
    {
      return (<div key={post.journalIndex}><JournalPostTemplate postData={post}/></div>)
    }
    ) : []) : (
    categories[currentCategory] ?
    categories[currentCategory].filter((el) => el.categoryIndex >= (currentPage - 1) * postsPerPage && el.categoryIndex < currentPage * postsPerPage)
    ?.map((post) => 
    {
      return (<div key={post.categoryIndex}><JournalPostTemplate postData={post}/></div>)
    }
    ) : [])
   }
    <div className={universalStyles.linkBox}>
    <Pagination 
      count={Math.ceil(Object.keys(categories[currentCategory] || {}).length / postsPerPage)} 
      onChange={(e, value) => setCurrentPage(value)} 
      variant="outlined" shape="rounded" sx={{"margin":"20px 0", "color":"rgb(148, 205, 171)"}}/>
    </div>
  </>
  );
